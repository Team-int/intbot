...
import { numberToKorean } from '../util/index'
import Discord from 'discord.js'
import { NewClient } from '../typings/Client'

async function find(str: string, client: NewClient) {
  var s = await client.stock.find().toArray()
  return s.filter((r: any) => r._id.includes(str) || r.name.includes(str) || r.code.includes(str))
}
...
export default {
  ...
  usage: '인트야 매수 <주식>',
  run: async (client: NewClient, message: any, args: string, ops: any) => {
    if (!args[1])
      return message.reply('사용법: ```인트야 매수 [주식] [숫자]```')
		
    const res = await find(args[1], client)
    const count = res.length
    const items = res.map((r: any) => r.name + '\n').join('')
    if (!res || res.length === 0) 
    ...
    const filter = (reaction: any, u: any) => reaction.emoji.name === '💳' && u.id === message.author.id
