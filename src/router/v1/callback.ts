...
import Oauth from '../../modules/DiscordOauth'

const callbackRouter = async (req: any, res: any) => {
  ...
  try {
    ...
    client.channels.cache.get('836917703075823636').send(
      Embed
        .setTitle('ERROR')
        .setColor('RED')
        .addField('요청 장소', req.path)
        .addField('오류내용', error.toString())
        .setTimestamp()
    )
  }
}
